...
class MazeStructure:
    ...
    def is_exit(self, pos_x, pos_y):
        return self.matrix[pos_y, pos_x] == OBSERVATION_MAPPING['EXIT']

    def is_path(self, pos_x, pos_y):
        return self.matrix[pos_y, pos_x] == OBSERVATION_MAPPING['PATH']

    def get_possible_neighbour_cords(self, pos_x, pos_y) -> tuple:
        """
    ...
    def create_graph(self):
        ...
        for n in path_nodes:
            neighbour_cells = self.get_possible_neighbour_cords(*n)
            allowed_cells = [c for c in neighbour_cells
                if self.is_path(*c) or self.is_exit(*c)]
            edges = [(n, dest) for dest in allowed_cells]
            g.add_edges_from(edges)
        return g

    def distinguish_direction(self, start, end):
        direction = ''
        if end[1] + 1 == start[1]:
            direction += 'N'
        if end[1] - 1 == start[1]:
    ...
    def get_all_possible_transitions(self):
        ...
        for node in path_nodes:
            for neighbour in nx.all_neighbors(g, node):
                direction = self.distinguish_direction(node, neighbour)
    ...
    def get_all_aliased_states(self):
        ...
        for x in range(0, self.max_x):
            for y in range(0, self.max_y):
                if self.aliasing_matrix[y, x] == OBSERVATION_MAPPING['ALIASING']:
    ...
    def get_all_non_aliased_states(self):
        ...
        for x in range(0, self.max_x):
            for y in range(0, self.max_y):
                if self.aliasing_matrix[y, x] == OBSERVATION_MAPPING['PATH']:
...
class MazeGymEnv(gym.Env):
    ...
    def __init__(self, matrix, aliasing_matrix, render_mode='aliasing_human'):
        self.maze = MazeStructure(matrix,aliasing_matrix)
    ...
    def _get_obs(self):
        return self.maze.build_perception_from_location(self._agent_location[0], self._agent_location[1])
        
    def _get_info(self):
        return {}

    def _get_reward(self):
        if self.maze.is_exit(self._agent_location[0], self._agent_location[1]):
            return 1000
        return 0

    def _is_terminated(self):
        return self.maze.is_exit(self._agent_location[0], self._agent_location[1])
