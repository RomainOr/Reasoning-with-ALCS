...
def build_fitness_matrix(env, population):
    original = env.unwrapped.maze.matrix
    ...
    for index, x in np.ndenumerate(original):
        ...
        if x == 0 or x == 3:
            perception = env.unwrapped.maze.build_perception_from_location(index[1], index[0])
...
def plot_policy(env, agent, ax=None, TITLE_TEXT_SIZE=18, AXIS_TEXT_SIZE=12):
    ...
    ax.set_aspect("equal")
    # Handy variables
    max_x = env.unwrapped.maze.max_x
    max_y = env.unwrapped.maze.max_y
